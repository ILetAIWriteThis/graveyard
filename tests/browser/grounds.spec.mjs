...
const computing = published.filter(item => item.domain === 'computing').length;
...
test('search includes preserved source, filters intersect, and empty state resets', async ({ page }) => {
  await page.goto('./');
  await page.locator('#search').fill('lstat');
  await expect(page.locator('.grave:visible')).toHaveCount(1);
  await expect(page.locator('.grave:visible')).toContainText('A pocket inventory');
  await page.getByRole('button', { name: 'Index', exact: true }).click();
  await expect(page.locator('.index-row:visible')).toHaveCount(1);
  await page.locator('#type-filter').selectOption('thought');
...
test('filters are linkable, history works, graph and related links navigate', async ({ page }) => {
  await page.goto('./?domain=computing&view=connections');
  await expect(page.locator('#result-count')).toHaveText(`${computing} of ${total} things found`);
  await expect(page.locator('.item-node')).toHaveCount(Math.min(computing, 30));
  await page.getByRole('button', { name: 'All corners' }).click();
  await expect(page.locator('.item-node')).toHaveCount(Math.min(total, 30));
  await page.goBack();
  await expect(page.locator('.item-node')).toHaveCount(Math.min(computing, 30));
  await page.getByRole('link', { name: 'A pocket inventory', exact: true }).click();
  await expect(page).toHaveURL(/\/graveyard\/items\/pocket-inventory\//);
  await expect(page.getByRole('heading', { name: 'WHAT', exact: true })).toBeVisible();
  await page.getByRole('link', { name: 'inventory.mjs Read source' }).click();
  await expect(page.locator('.source-code')).toContainText("from 'node:fs/promises'");
});
...
test('keyboard search shortcut and reduced-motion setting work', async ({ page }) => {
  ...
  await page.keyboard.type('compost');
  await expect(page.locator('.grave:visible')).toHaveCount(1);
  await page.keyboard.press('Escape');
  await expect(page.locator('.grave:visible')).toHaveCount(total);
...
test('navigation and source reading work without JavaScript', async ({ browser }) => {
  ...
  await page.locator('.grave[data-id="pocket-inventory"]').click();
  await expect(page.getByRole('heading', { name: 'A pocket inventory', exact: true })).toBeVisible();
  await page.getByRole('link', { name: 'inventory.mjs Read source' }).click();
...
test('dark and daylight grounds, graph, and reading page pass accessibility checks', async ({ browser }) => {
  ...
  for (const theme of ['dark', 'light']) {
    for (const route of ['', '?view=connections', 'items/pocket-inventory/']) {
